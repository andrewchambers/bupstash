...
from a hash lookup. while also allowing content addressed based deduplication. As client keys do not know eachother's hash
key, client's cannot intentionally corrupt eachothers keyspace with bad data, while the master key can still verify data
integrity using this hash unique function.
