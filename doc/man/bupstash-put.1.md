...
* BUPSTASH_CHECKPOINT_SECONDS:
  When send logging is enabled, bupstash will checkpoint the log approximately every BUPSTASH_CHECKPOINT_SECONDS.
  If an upload is interrupted after a successful checkpoint, data will not need
  to be resent over the network. The default value of this option is 300, which is 5 minutes.
